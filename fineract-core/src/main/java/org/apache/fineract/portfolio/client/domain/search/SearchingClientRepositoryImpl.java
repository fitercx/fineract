...
public class SearchingClientRepositoryImpl implements SearchingClientRepository {
    ...
    public Page<SearchedClient> searchByText(String searchText, Pageable pageable, String officeHierarchy) {
        ...
        Specification<Client> spec = (r, q, builder) -> {
            ...
            predicates.add(cb.or(cb.like(cb.lower(r.get("accountNumber")), searchLikeValue), cb.like(cb.lower(r.get("displayName")), searchLikeValue),
                    cb.like(cb.lower(r.get("externalId")), searchLikeValue), cb.like(cb.lower(r.get("mobileNo")), searchLikeValue),
