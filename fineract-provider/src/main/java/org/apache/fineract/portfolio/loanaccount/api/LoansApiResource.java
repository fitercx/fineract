...
public class LoansApiResource {
    ...
    public String retrieveAll(@Context final UriInfo uriInfo,
            ...
            @QueryParam("status") @Parameter(description = "status") final String status, @QueryParam("locId") final Long locId) {
        ...
        final SearchParameters searchParameters = SearchParameters.builder().accountNo(accountNo).sortOrder(sortOrder)
                .externalId(externalId).offset(offset).limit(limit).orderBy(orderBy).status(status).clientId(clientId).locId(locId).build();
