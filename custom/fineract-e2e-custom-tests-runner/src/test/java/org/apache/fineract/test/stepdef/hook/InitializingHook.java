...
/**
 * Custom InitializingHook that uses our ordered initialization
 */
@Slf4j
...
public class InitializingHook {
    ...
    public static void beforeAll() throws Exception {
        ...
        environment.getPropertySources().addFirst(new ResourcePropertySource("classpath:fineract-custom-test-application.properties"));
        ...
        if (initializerProperties.isEnabled()) {
            log.info("Running custom ordered initialization for Fineract");
            FineractInitializerFactory.get().setupGlobalDefaults();
        } else {
            log.info("Skipping initialization for Fineract");
        }
    ...
    public static void before() throws Exception {
        log.info("Custom InitializingHook.before() - Setting up scenario");
        FineractInitializerFactory.get().setupDefaultsForScenario();
    }

    @AfterAll
    public static void afterAll() throws Exception {
        log.info("Custom InitializingHook.afterAll() - Cleaning up");
