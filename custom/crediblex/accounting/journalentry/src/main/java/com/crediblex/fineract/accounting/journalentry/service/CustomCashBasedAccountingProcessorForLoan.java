        ...
        if (penaltiesAmount != null && penaltiesAmount.compareTo(BigDecimal.ZERO) > 0) {
            ...
            if (isRBFProduct(loanProductId)) {
                ...
                if (rbfPenaltyIncomeAccount != null) {
                    log.info("CustomCashBasedAccountingProcessorForLoan: Using GL 300015 (Over Due Interest - LPI - RBF) for RBF penalty income, amount: {}", penaltiesAmount);
                    this.helper.createCreditJournalEntryForLoan(office, currencyCode, loanId, transactionId, transactionDate,
                            penaltiesAmount, rbfPenaltyIncomeAccount);
                } else {
                    log.warn("CustomCashBasedAccountingProcessorForLoan: GL 300015 (Over Due Interest - LPI - RBF) not found for RBF product {}. Falling back to default INCOME_FROM_PENALTIES.", loanProductId);
                    this.helper.createCreditJournalEntryForLoanCharges(office, currencyCode,
