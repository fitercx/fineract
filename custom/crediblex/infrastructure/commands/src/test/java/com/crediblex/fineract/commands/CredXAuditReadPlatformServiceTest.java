...
public class CredXAuditReadPlatformServiceTest {
    ...
    void retrieveAllEntriesToBeChecked_withWaiveCharges_shouldEnhanceData() {
        ...
        when(auditReadPlatformService.retrieveAllEntriesToBeChecked(any(SQLBuilder.class), any(Boolean.class))).thenReturn(originalList);
        when(loanChargeWaiverRepository.fetchLoanChargeWaiverDetails(anyList())).thenReturn(waiveDetails);
    ...
    void retrieveAllEntriesToBeChecked_withWaiveChargesButNoMatchingDetails_shouldReturnOriginalData() {
        ...
        when(loanChargeWaiverRepository.fetchLoanChargeWaiverDetails(anyList())).thenReturn(Collections.emptyList());
    ...
    void retrieveAllEntriesToBeChecked_withPartialMatchingDetails_shouldEnhanceOnlyMatchingData() {
        ...
        when(auditReadPlatformService.retrieveAllEntriesToBeChecked(any(SQLBuilder.class), any(Boolean.class))).thenReturn(originalList);
        when(loanChargeWaiverRepository.fetchLoanChargeWaiverDetails(List.of(100L, 200L))).thenReturn(waiveDetails);
    ...
    private AuditData createAuditData(Long id, String actionName, String entityName, Long resourceId) {
        return new AuditData(id, actionName, entityName, resourceId, null, "maker", ZonedDateTime.now(), null, null, "success", "{}",
                "Office 1", null, null, "Client Name", "LN-001", null, 1L, 1L, "/api/url");
    }

    private AuditQueries.LoanChargeWaiveDetails.Result createWaiveDetail(Long chargeId, String clientName, Long loanId,
            BigDecimal waiveAmount) {
