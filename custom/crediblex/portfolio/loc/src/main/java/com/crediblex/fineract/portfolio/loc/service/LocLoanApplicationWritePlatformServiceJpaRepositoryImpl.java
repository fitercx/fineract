...
/**
 * Custom service that provides Line of Credit functionality for loan application submission. This service can be used
 * to enhance loan applications with line of credit associations.
 */
@Service
@Slf4j
public class LocLoanApplicationWritePlatformServiceJpaRepositoryImpl {
    ...
    @Autowired
    public LocLoanApplicationWritePlatformServiceJpaRepositoryImpl(FromJsonHelper fromApiJsonHelper,
    ...
    /**
     * Process loan application with Line of Credit functionality. This method can be called after a loan application is
     * submitted to associate it with a line of credit.
     *
     * @param command
     *            the JSON command containing loan application data
     * @param loanId
     *            the ID of the created loan
        ...
        if (lineOfCreditId != null) {
            try {
                ...
                if (success) {
                    ...
                    log.warn("Failed to associate loan {} with line of credit {} - association returned false", loanId, lineOfCreditId);
                    return false;
                }
            } catch (Exception e) {
                log.error("Failed to associate loan {} with line of credit {}: {}", loanId, lineOfCreditId, e.getMessage());
                return false;
            }
        } else {
