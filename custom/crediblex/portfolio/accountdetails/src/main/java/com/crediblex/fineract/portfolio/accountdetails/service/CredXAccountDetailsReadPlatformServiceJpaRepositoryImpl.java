    ...
    private static final class CredXLoanAccountSummaryDataMapper implements RowMapper<LoanAccountSummaryData> {
        ...
        public String loanAccountSummarySchema() {
            ...
            accountsSummary.append(" l.product_id as productId, lp.name as productName, lp.short_name as shortProductName,").append(
                    ...
                    .append(" AND rps.installment > 0 AND rps.is_down_payment = false) ")
                    ...
                    .append(" WHERE ls2.loan_id = l.id ")
                    .append(" AND (ls2.completed_derived = false OR ls2.completed_derived IS NULL) ")
                    .append(" AND ls2.installment > 0 AND ls2.is_down_payment = false) ")
                    .append(" ORDER BY ls.installment ASC LIMIT 1) ")
                    ...
                    .append(" + COALESCE(ls.penalty_charges_amount, 0) ")
                    .append(" FROM m_loan_repayment_schedule ls ")
                    .append(" WHERE ls.loan_id = l.id ")
                    .append(" AND (ls.completed_derived = false OR ls.completed_derived IS NULL) ")
                    .append(" AND ls.installment > 0 AND ls.is_down_payment = false ")
