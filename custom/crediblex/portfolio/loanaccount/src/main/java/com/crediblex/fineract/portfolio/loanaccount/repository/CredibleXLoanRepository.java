...
public interface CredibleXLoanRepository extends JpaRepository<Loan, Long>, JpaSpecificationExecutor<Loan> {
    @Query("SELECT loan FROM Loan loan " +
            "WHERE loan.client.office.id IN :officeIds " +
            "AND loan.loanStatus IN :loanStatuses " +
            "AND loan.id >= :maxLoanIdInList")
    Page<Loan> findByClientOfficeIdsAndLoanStatusWithPagination(
    ...
    Page<Loan> customFindByGroupOfficeIdsAndLoanStatusWithPagination(
            @Param("officeIds") Collection<Long> officeIds,
            @Param("loanStatuses") Collection<LoanStatus> loanStatuses,
            @Param("maxLoanIdInList") Long maxLoanIdInList,
            Pageable pageable);
