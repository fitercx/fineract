...
/**
 * CredX processor for RBI strategy that uses {@link CredXLoanRepaymentScheduleProcessingWrapper} so LPI is assigned to
 * the same month's row.
 *
 * <p>
...
public class CredXRBILoanRepaymentScheduleTransactionProcessor extends RBILoanRepaymentScheduleTransactionProcessor {
    ...
    public CredXRBILoanRepaymentScheduleTransactionProcessor(ExternalIdFactory externalIdFactory,
    ...
    protected LoanRepaymentScheduleProcessingWrapper createLoanRepaymentScheduleProcessingWrapper(
        ...
        if (disbDate != null && lpiSameMonthProperties != null && lpiSameMonthProperties.isEnabledForDisbursementDate(disbDate)) {
            log.debug("LPI same-month: using CredX wrapper for disbursementDate={} (RBI)", disbDate);
