...
public class CredXLoanReadPlatformServiceImpl extends LoanReadPlatformServiceImpl {
    ...
    public LoanScheduleData retrieveRepaymentSchedule(final Long loanId,
        ...
        try {
            ...
            final LoanScheduleResultSetExtractor fullResultsetExtractor = new LoanScheduleResultSetExtractor(
                    ...
                    customReversedChargeCalculationService);
            ...
            Collection<ExtendedLoanSchedulePeriodData> periodDataWithStatus = periods.stream().map(p -> {
                BigDecimal reversedPenaltyCharges = calculateReversedPenaltyChargesForPeriod(loan, p);
                return new ExtendedLoanSchedulePeriodData(p, resolvePeriodStatus(currency, p), reversedPenaltyCharges);
            }).toList();
    ...
    private BigDecimal calculateReversedPenaltyChargesForPeriod(Loan loan, LoanSchedulePeriodData period) {
        if (period.getPeriod() == null) {
            // Disbursement period - no reversed charges
            return BigDecimal.ZERO;
        }

        BigDecimal reversedAmount = BigDecimal.ZERO;
        Integer installmentNumber = period.getPeriod();
        LocalDate periodDueDate = period.getDueDate();

        if (periodDueDate == null) {
            log.warn("Period {} has null due date, cannot match reversed charges", installmentNumber);
            return BigDecimal.ZERO;
        }
            ...
            if (loanCharge.isPenaltyCharge() && !loanCharge.isActive()) {
                ...
                if (hasChargeAdjustment) {
                    ...
                    if (loanCharge.isOverdueInstallmentCharge() && loanCharge.getDueLocalDate() != null) {
                        ...
                        LoanInstallmentCharge installmentCharge = loanCharge.getInstallmentLoanCharge(installmentNumber);
                        appliesToPeriod = (installmentCharge != null);
                        if (appliesToPeriod) {
                            log.debug("Matched reversed installment fee charge {} to period {} via LoanInstallmentCharge",
                                    loanCharge.getId(), installmentNumber);
                        }
                    }
                    ...
                    if (appliesToPeriod) {
                        ...
                        if (originalPaidAmount.compareTo(BigDecimal.ZERO) > 0) {
                            ...
                            log.info("Added reversed charge {} (due: {}) amount {} to period {} (due: {}), total reversed: {}",
        ...
        if (reversedAmount.compareTo(BigDecimal.ZERO) > 0) {
            log.info("Period {} (due: {}) has total reversed penalty charges: {}", installmentNumber, periodDueDate, reversedAmount);
        }
    ...
    private final class CredibleXLoanMapper implements RowMapper<LoanAccountData> {
        ...
        private void extractLocDetails(ExtendedLoanAccountData extendedLoanAccountData, ResultSet rs) throws SQLException {
            ...
            final BigDecimal locApprovedInvoiceAmountInAED = rs.getBigDecimal(LoanAccountAdditionalProperties.APPROVED_INVOICE_AMOUNT_IN_AED);
            ...
            final BigDecimal locFundedAmountInInvoiceCurrency = rs.getBigDecimal(LoanAccountAdditionalProperties.FUNDED_AMOUNT_IN_INVOICE_CURRENCY);
            ...
            if (locDisapprovedAmountInAED != null) {
                extendedLoanAccountData.addCustomParameter(LoanAccountAdditionalProperties.DISAPPROVED_AMOUNT_IN_AED, locDisapprovedAmountInAED);
            }
            if (locApprovedInvoiceAmountInAED != null) {
                extendedLoanAccountData.addCustomParameter(LoanAccountAdditionalProperties.APPROVED_INVOICE_AMOUNT_IN_AED, locApprovedInvoiceAmountInAED);
            }
            if (locAmountAfterAdvanceInAED != null) {
                extendedLoanAccountData.addCustomParameter(LoanAccountAdditionalProperties.AMOUNT_AFTER_ADVANCE_IN_AED, locAmountAfterAdvanceInAED);
            }
            if (locRequestedAmountInAED != null) {
                extendedLoanAccountData.addCustomParameter(LoanAccountAdditionalProperties.REQUESTED_AMOUNT_IN_AED, locRequestedAmountInAED);
            }
            if (locFundedAmountInInvoiceCurrency != null) {
                extendedLoanAccountData.addCustomParameter(LoanAccountAdditionalProperties.FUNDED_AMOUNT_IN_INVOICE_CURRENCY, locFundedAmountInInvoiceCurrency);
            }
    ...
    private LoanSummaryData correctSummaryForVatAtDisbursement(final Long loanId, final LoanSummaryData loanSummary) {
        ...
        // 1) Derive the pure fee portion from active disbursement charges
        ...
        // 2) Derive VAT actually paid at disbursement from loan transactions,
        // considering only non-reversed VAT_DEDUCTION_AT_DISBURSEMENT transactions.
        // This ensures VAT from undone disbursements is excluded.
        for (LoanTransaction transaction : loan.getLoanTransactions()) {
            if (transaction.getTypeOf().isVatDeductionAtDisbursement() && transaction.isNotReversed()) {
                vatPaidAtDisbursement = vatPaidAtDisbursement.add(transaction.getAmount(loan.getCurrency()).getAmount());
            }
        }
        ...
        BigDecimal feeChargesDueAtDisbursementCharged = loanSummary.getFeeChargesDueAtDisbursementCharged() != null
                ? loanSummary.getFeeChargesDueAtDisbursementCharged()
                : BigDecimal.ZERO;

        // After undo last disbursement, core can still include reversed tranche fee in summary; use only valid
        // (disbursed) amounts
        BigDecimal newFeeChargesCharged = feeChargesCharged;
        BigDecimal newFeeChargesDueAtDisbursementCharged = feeChargesDueAtDisbursementCharged;
        if (principalCorrected) {
            newFeeChargesCharged = feeOnlyFromCharges;
            newFeeChargesDueAtDisbursementCharged = feeOnlyFromCharges.add(vatPaidAtDisbursement);
        }
        ...
        if (!principalCorrected && feeChargesCharged.compareTo(feeOnlyFromCharges.add(vatPaidAtDisbursement)) >= 0) {
        ...
        } else if (principalCorrected) {
            newTaxChargesCharged = vatPaidAtDisbursement;
        }
        // If core already included VAT in feeChargesPaid (feeChargesPaid >= fee + VAT), reclassify only; else add VAT
        // to total. When principalCorrected (undo), paid amounts must reflect only non-reversed tranche.
        BigDecimal newFeeChargesPaid;
        BigDecimal newTaxChargesPaid;
        BigDecimal newTotalRepayment;
        BigDecimal newTotalCostOfLoan;
        if (principalCorrected) {
            newFeeChargesPaid = feeOnlyFromCharges;
            newTaxChargesPaid = vatPaidAtDisbursement;
            // Total repayment/cost: replace core's fee+tax paid with actual disbursed-tranche-only (fee + VAT)
            BigDecimal disbursementChargesPaid = feeOnlyFromCharges.add(vatPaidAtDisbursement);
            newTotalRepayment = totalRepayment.subtract(feeChargesPaid).subtract(taxPaid).add(disbursementChargesPaid);
            newTotalCostOfLoan = totalCostOfLoan.subtract(feeChargesPaid).subtract(taxPaid).add(disbursementChargesPaid);
        } else if (feeChargesPaid.compareTo(feeOnlyFromCharges.add(vatPaidAtDisbursement)) >= 0) {
            ...
            newTaxChargesPaid = taxPaid.add(vatPaidAtDisbursement);
        ...
        return LoanSummaryData.builder().currency(loanSummary.getCurrency()).principalDisbursed(principalDisbursedToUse)
                ...
                .feeAdjustments(loanSummary.getFeeAdjustments()).feeChargesDueAtDisbursementCharged(newFeeChargesDueAtDisbursementCharged)
                .feeChargesPaid(newFeeChargesPaid).feeChargesWaived(loanSummary.getFeeChargesWaived())
                .feeChargesWrittenOff(loanSummary.getFeeChargesWrittenOff()).feeChargesOutstanding(loanSummary.getFeeChargesOutstanding())
                .feeChargesOverdue(loanSummary.getFeeChargesOverdue()).penaltyChargesCharged(loanSummary.getPenaltyChargesCharged())
                .penaltyAdjustments(loanSummary.getPenaltyAdjustments()).penaltyChargesPaid(loanSummary.getPenaltyChargesPaid())
                .penaltyChargesWaived(loanSummary.getPenaltyChargesWaived())
    ...
    private static final class LoanScheduleResultSetExtractor implements ResultSetExtractor<LoanScheduleData> {
        ...
        LoanScheduleResultSetExtractor(final RepaymentScheduleRelatedLoanData repaymentScheduleRelatedLoanData,
                ...
                Set<LoanCharge> loanCharges, CustomReversedChargeCalculationService customReversedChargeCalculationService) {
        ...
        public LoanScheduleData extractData(@NotNull final ResultSet rs) throws SQLException, DataAccessException {
            ...
            while (rs.next()) {
                ...
                if (shouldUseExpectedDisbursementBalance(isDisbursementPeriod, principalDue)) {
                ...
                final LoanSchedulePeriodData periodData = LoanSchedulePeriodData.periodWithPayments(period, fromDate, dueDate,
                        ...
                        feeChargesReversed, penaltyChargesReversed);
        ...
        private BigDecimal processDisbursementData(LoanScheduleType loanScheduleType, Collection<DisbursementData> disbursementData,
            ...
            for (final DisbursementData data : disbursementData) {
                ...
                if (((fromDate.equals(this.disbursement.disbursementDate()) && data.disbursementDate().equals(fromDate))
                    ...
                    if (!isPositive(chargesForThisTranche)) {
        ...
        private LoanSchedulePeriodData createLoanSchedulePeriodData(final DisbursementData data, BigDecimal disbursementChargeAmount,
            ...
            if (isPositive(feeChargesDueFromDB)) {
        ...
        private BigDecimal recalculateDisbursementChargesForTranche(final BigDecimal trancheAmount, final BigDecimal totalOriginalPrincipal,
            ...
            for (LoanCharge loanCharge : loanCharges) {
                ...
                if (isPercentageBased) {
                    ...
                    if (isPositive(totalMultiDisbursed) && totalChargeAmount.isGreaterThanZero()) {
                ...
                if (loanCharge.hasTax()) {
                    ...
                    if (totalTaxAmount.isGreaterThanZero()) {
                        ...
                        if (loanCharge.getChargeCalculation().isPercentageBased()) {
                            ...
                            if (isPositive(totalMultiDisbursed)) {
