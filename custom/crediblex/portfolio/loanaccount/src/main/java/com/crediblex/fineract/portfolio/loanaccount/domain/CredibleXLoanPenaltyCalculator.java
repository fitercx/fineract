...
public class CredibleXLoanPenaltyCalculator {
    ...
    public CredibleXLoanPenaltyCalculator(List<ExtendedLoanSchedulePeriodData> periods,
                                          Collection<LoanChargeData> loanCharges, long penaltyWaitPeriodValue) {
...
// Collect the included charges (unpaid, applicable penalties)
        List<LoanChargeData> includedCharges = loanCharges.stream()
...
// Calculate the penalty sum from unpaid, applicable penalties
    ...
    public Collection<LoanChargeData> getApplicableCharges(LocalDate transactionDate) {
        ...
        return loanCharges.stream()
                ...
                .filter(charge -> charge.getAmountPaid() == null
                        || charge.getAmountPaid().compareTo(charge.getAmount()) < 0) // exclude fully paid charges
    ...
    public List<LoanChargeData> getPenaltiesToDisable(final LocalDate transactionDate, final Long loanId) {
        ...
        if (applicableCharges.isEmpty()) {
            ...
            return loanCharges.stream()
                    .filter(LoanChargeData::isPenalty)
                    .sorted(Comparator.comparing(LoanChargeData::getDueDate))
                    .collect(Collectors.toList());
        }

        // Step 2: Find the latest due date among applicable charges
        LocalDate latestApplicableDueDate = applicableCharges.stream()
                .map(LoanChargeData::getDueDate)
        ...
        return loanCharges.stream()
                .filter(LoanChargeData::isPenalty) // only penalties
                .filter(charge -> charge.getDueDate() != null
                        && charge.getDueDate().isAfter(latestApplicableDueDate)) // after latest applicable
