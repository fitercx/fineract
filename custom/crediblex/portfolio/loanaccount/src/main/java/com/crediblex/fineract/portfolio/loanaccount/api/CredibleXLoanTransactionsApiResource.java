...
public class CredibleXLoanTransactionsApiResource extends LoanTransactionsApiResource {
    ...
    @Operation(
            summary = "Retrieve Penalties Accrued up to a Given Date",
            description = "Returns only the penalties accrued on a loan up to the provided transaction date. "
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "OK",
                    content = @Content(schema = @Schema(
                            implementation = BackdatedRepaymentPenaltyDTO.class
                    )))
    })
    public String retrievePenaltiesTemplate(
        ...
        BackdatedRepaymentPenaltyDTO penaltiesData =
                this.credibleXLoanReadPlatformService.retrieveLoanPenaltiesTemplate(loanId, transactionDate);
