...
public class LineOfCreditDataValidator {
    ...
    public void validateForDraw(String json) {
        if (json == null) {
            throw new InvalidJsonException();
        }

        final List<ApiParameterError> dataValidationErrors = new ArrayList<>();
        final DataValidatorBuilder baseDataValidator = new DataValidatorBuilder(dataValidationErrors).resource("line.of.credit.draw");
        ...
        if (this.fromApiJsonHelper.parameterExists("referenceNumber", element)) {
            final String referenceNumber = this.fromApiJsonHelper.extractStringNamed("referenceNumber", element);
            baseDataValidator.reset().parameter("referenceNumber").value(referenceNumber).notExceedingLengthOf(100);
        }
    ...
    public void validateForRepayment(String json) {
        if (json == null) {
            throw new InvalidJsonException();
        }

        final List<ApiParameterError> dataValidationErrors = new ArrayList<>();
        final DataValidatorBuilder baseDataValidator = new DataValidatorBuilder(dataValidationErrors).resource("line.of.credit.repayment");
