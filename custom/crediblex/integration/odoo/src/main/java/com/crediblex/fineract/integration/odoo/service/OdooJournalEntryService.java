...
public class OdooJournalEntryService {
    ...
    public Map<Integer, Long> postJournalEntriesForLoan(Long loanId, List<JournalEntryOdooSync> journalEntryOdooSyncs) {
        ...
        try {
            ...
            // Group entries by journal using business event type from tracking records and loan product type
    ...
    private Map<Integer, List<JournalEntry>> groupEntriesByJournal(List<JournalEntryOdooSync> journalEntryOdooSyncs, Long loanId) {
        ...
        for (JournalEntryOdooSync sync : journalEntryOdooSyncs) {
            ...
            Integer journalId = odooIntegrationService.getJournalIdForGlCode(glCode, businessEventType, isDebit, productType);
    ...
    /**
     * Get product type from loan external ID
     */
    private String getProductTypeFromLoanId(Long loanId) {
        if (loanId == null) {
            return OdooIntegrationReadPlatformServiceImpl.REVENUE_BASED_FINANCING; // Default
        }
        ...
        try {
            ...
            if (!externalIds.isEmpty()) {
                ...
                if (externalId != null && !externalId.trim().isEmpty()) {
                    log.debug("Retrieved product type '{}' for loan ID: {} from product external_id", externalId, loanId);
                    return externalId;
                }
            }
    ...
    public Long postEarlyClosureJournalEntriesToOdoo(Long loanId, List<Map<String, Object>> transformedJournalLines,
        ...
        try {
            ...
            if (transformedJournalLines.isEmpty()) {
                log.warn("No transformed journal lines provided for loan {}", loanId);
                return null;
            }
            ...
            Integer uid = odooApiClient.authenticate();
            if (uid == null) {
                log.error("Failed to authenticate with Odoo for early closure entries - loan {}", loanId);
                return null;
            }

            // Use the BNK8 journal for early closure entries (journal ID 24)
            Integer journalId = odooIntegrationService.getJournalIdByOdooCode(odooJournalCode);
    ...
    private Map<String, Object> buildAccountMoveValuesForEarlyClosure(Long loanId, List<Map<String, Object>> transformedJournalLines,
        ...
        for (Map<String, Object> journalLine : transformedJournalLines) {
            ...
            if (accountId != null) {
                ...
                log.warn("Could not find account ID for GL code: {} in early closure for loan {}", glCode, loanId);
            }
        }
    ...
    private Integer getAccountIdByGlCode(String glCode) {
        try {
            return odooIntegrationService.getOdooAccountId(glCode);
        } catch (Exception e) {
            log.error("Error getting Odoo account ID for GL code {}: {}", glCode, e.getMessage());
            return null;
        }
    }
    ...
    public boolean doesAccountExistInOdoo(String glCode) {
        if (glCode == null || glCode.trim().isEmpty()) {
            log.warn("Cannot check empty or null GL code in Odoo");
            return false;
        }
