...
public class OdooJournalEntryService {
    ...
    private List<Object> buildConsolidatedMoveLines(List<JournalEntry> journalEntries) {
        ...
        for (JournalEntry entry : journalEntries) {
            ...
            if (accountId == null) {
                String errorMsg = String.format("Could not map GL account '%s' to Odoo account ID for entry %d. Check account mapping configuration.", 
                        accountCode, entry.getId());
                log.error(errorMsg);
            ...
            if (!doesAccountExistInOdoo(accountCode)) {
                String errorMsg = String.format("GL account '%s' (mapped to account ID %d) does not exist in Odoo for entry %d. Account may have been deleted or archived.", 
            ...
            log.debug("Processed journal entry {} with GL code '{}' mapped to account ID {}", 
                    entry.getId(), accountCode, accountId);
        }
